...
public class ProxyInstanceStepsFactory extends AbstractStepsFactory {
    ...
    public ProxyInstanceStepsFactory(Configuration configuration, List<?> stepsInstances) {
        ...
        while(var3.hasNext()) {
            ...
            // use superclass for all step instances under package de.tallence.advancedBDD . Because we use a spring aop proxy arround them. Otherwise jbehave does not find the step methods via class.getMethods();
            this.stepsInstances.put( instance.getClass().getSuperclass().getPackage().getName().startsWith( "de.tallence.advancedBDD" ) ? instance.getClass().getSuperclass() : instance.getClass(), instance);
        }
